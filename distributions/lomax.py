...
__all__ = [
    "Lomax",
    "LomaxWithSoftplusConcentrationScale",
]
...
class Lomax(distribution.Distribution):
  ...
  * `Z` is the normalizsing constant.
  ...
  def _variance(self):
    variance = math_ops.square(self.scale) * (self.concentration - 1) \
      / (math_ops.square(self.concentration - 1) * (self.concentration - 2))
    if self.allow_nan_stats:
      nan = array_ops.fill(
          self.batch_shape_tensor(),
          np.array(np.nan, dtype=self.dtype.as_numpy_dtype()),
      ...
      inf = array_ops.fill(
          ...
          name="inf")
      return array_ops.where(self.concentration > 2., variance, #nan)
      ...
      return control_flow_ops.with_dependencies([
          check_ops.assert_less(
              array_ops.ones([], self.dtype),
              self.concentration,
              message="standard deviation not defined when any concentration <= 1"),
          ], mode)
...
class LomaxWithSoftplusConcentrationScale(Lomax):
  """`Lomax` with softplus of `concentration` and `scale`."""
